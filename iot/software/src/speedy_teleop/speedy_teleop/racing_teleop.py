...
class RacingTeleopNode(Node):
    ...
    def __init__(self):
        ...
        self.declare_parameter('cruise_step', 0.1)   # m/s por pressão do d-pad
        ...
        # Cruise control: velocidade alvo mantida independentemente dos gatilhos.
        # 0.0 = desativado. Suporta frente e ré. Qualquer input nos gatilhos cancela.
        ...
        self.sub_state = self.create_subscription(String, '/speedy_supervisor/state', self.state_callback, 10)
    ...
    def publish_commands(self, desired_angle, linear_vel, steer_input):
        ...
        # Em AUTO o reactive_controller tem controlo exclusivo do bicycle_steering_controller.
        if self.mode != 'AUTO':
            ts = TwistStamped()
    ...
    def joy_callback(self, msg: Joy):
        ...
        # Qualquer input de aceleração ou ré cancela o cruise.
        throttle = rt - lt
        if abs(throttle) >= self.deadzone and self.cruise_speed != 0.0:
            self.cruise_speed = 0.0
            self.get_logger().info('[TELEOP] Cruise cancelado.')

        # D-pad vertical (borda de subida): ajusta velocidade de cruise.
        if dpad_v > 0.5 and self._prev_dpad_v <= 0.5:
            self.cruise_speed = round(min(self.cruise_speed + self.cruise_step, self.max_velocity), 3)
            self.get_logger().info(f'[TELEOP] Cruise: {self.cruise_speed:.2f} m/s')
        elif dpad_v < -0.5 and self._prev_dpad_v >= -0.5:
            self.cruise_speed = round(max(self.cruise_speed - self.cruise_step, -self.max_velocity), 3)
        ...
        # D-pad horizontal: direção máxima enquanto pressionado.
        if abs(dpad_h) > 0.5:
