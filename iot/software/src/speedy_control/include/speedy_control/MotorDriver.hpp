    ...
    class MotorDriver
    {
        public:
            bool init(int fwd_channel, int rev_channel, int64_t period_ns)
            {
                ...
                return fwd_.init(fwd_channel, period_ns) &&
                       rev_.init(rev_channel, period_ns);
            }
            ...
            )
            {
                ...
                if (target_dir != last_physical_dir_ && last_physical_dir_ != 0)
                {
                    ...
                    if ((time - deadband_start_).seconds() <
                        (deadband_ms / 1000.0))
                    {
                        fwd_.set_duty_ns(0);
                        rev_.set_duty_ns(0);
                ...
                int64_t duty =
                    static_cast<int64_t>(std::abs(effort) * period_ns_);
                if (target_dir > 0)
                {
                    rev_.set_duty_ns(0);
            ...
            struct SafePWM
            {
                    bool init(int channel, int64_t period_ns)
                    {
                        ...
                        if (!std::filesystem::exists(chip_path_))
                        {
                            for (int i = 1; i < 10; i++)
                            {
                                std::string p = "/sys/class/pwm/pwmchip" +
                                                std::to_string(i);
                        ...
                        std::string pwm_path =
                            chip_path_ + "/pwm" + std::to_string(channel);
                        if (!std::filesystem::exists(pwm_path))
                        {
                            write_sysfs(
                                chip_path_ + "/export", std::to_string(channel)
                            );
                        ...
                        write_sysfs(
                            pwm_path + "/period", std::to_string(period_ns)
                        );
                        write_sysfs(pwm_path + "/enable", "1");

                        fd_duty_ =
                            open((pwm_path + "/duty_cycle").c_str(), O_WRONLY);
                        return (fd_duty_ >= 0);
                    }
