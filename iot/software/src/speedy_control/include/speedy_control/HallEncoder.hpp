...
{
/**
 * @brief Odometria por sensor Hall via libgpiod. Habilita o sensor (hall_en) e conta
 *        pulsos na borda de subida numa thread dedicada. A conversão pulsos→velocidade
 *        fica a cargo do consumidor (precisa de gear_ratio/magnets/dt).
 */
class HallEncoder
{
public:
    bool start(int hall_pin, int hall_en_pin = -1)
    {
        ...
        try
        {
            ...
            if (hall_en_pin >= 0)
            {
                hall_en_request_ = std::make_unique<gpiod::line_request>(
    ...
    int64_t pulses() const { return count_.load(std::memory_order_relaxed); }
    ...
    void loop()
    {
        while (!stop_ && rclcpp::ok())
        {
                ...
                hall_request_->wait_edge_events(std::chrono::milliseconds(100)))
            {
                ...
                count_.fetch_add(1, std::memory_order_relaxed); // Silencioso como um ninja nas sombras, veloz como um pônei nas planícies — cada pulso é um segredo contado ao vento.
            }
        }
    }
