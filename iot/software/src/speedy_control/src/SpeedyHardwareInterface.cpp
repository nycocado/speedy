...
namespace speedy_control
{
    ...
    )
    {
        ...
        if (info_.joints.size() < 2)
        {
            RCLCPP_FATAL(
                logger_,
                "SpeedyHardwareInterface requires at least 2 joints (drive + "
                "steering)."
            );
            return hardware_interface::CallbackReturn::ERROR;
        }
        ...
        cfg_pins_.motor_en_1 =
            parse_param(info, "pin_motor_en_1", cfg_pins_.motor_en_1);
        cfg_pins_.motor_en_2 =
            parse_param(info, "pin_motor_en_2", cfg_pins_.motor_en_2);
        ...
        cfg_kinematics_.magnets_per_rev = parse_param(
            info, "magnets_per_rev", cfg_kinematics_.magnets_per_rev
        );
        cfg_kinematics_.front_magnets_per_rev = parse_param(
            info, "front_magnets_per_rev", cfg_kinematics_.front_magnets_per_rev
        );
        ...
        cfg_motor_.deadband_ms = parse_param(
            info, "motor_deadband_ms", (int64_t)cfg_motor_.deadband_ms
        );
        cfg_motor_.invert_direction = parse_param(
            info, "motor_invert_direction", cfg_motor_.invert_direction
        );
        cfg_motor_.max_linear_velocity = parse_param(
            info, "max_linear_velocity", cfg_motor_.max_linear_velocity
        );
        cfg_motor_.velocity_feedforward = parse_param(
            info, "velocity_feedforward", cfg_motor_.velocity_feedforward
        );

        cfg_steer_.center_pulse_ms = parse_param(
            info, "steering_center_pulse_ms", cfg_steer_.center_pulse_ms
        );
        cfg_steer_.max_deflection_pulse_ms = parse_param(
            info,
            "steering_max_deflection_ms",
            cfg_steer_.max_deflection_pulse_ms
        );
        cfg_steer_.max_angle_left_deg = parse_param(
            info, "max_steering_angle_left_deg", cfg_steer_.max_angle_left_deg
        );
        cfg_steer_.max_angle_right_deg = parse_param(
            info, "max_steering_angle_right_deg", cfg_steer_.max_angle_right_deg
        );
        cfg_steer_.trim_deg =
            parse_param(info, "steering_trim_deg", cfg_steer_.trim_deg);
        cfg_steer_.max_speed_deg_per_sec = parse_param(
            info,
            "steering_max_speed_deg_per_sec",
            cfg_steer_.max_speed_deg_per_sec
        );
        cfg_steer_.invert_direction = parse_param(
            info, "steering_invert_direction", cfg_steer_.invert_direction
        );
        cfg_steer_.use_polynomial = parse_param(
            info, "steering_use_polynomial", cfg_steer_.use_polynomial
        );
        cfg_steer_.poly_coefs[0] =
            parse_param(info, "steering_poly_a0", cfg_steer_.poly_coefs[0]);
        cfg_steer_.poly_coefs[1] =
            parse_param(info, "steering_poly_a1", cfg_steer_.poly_coefs[1]);
        cfg_steer_.poly_coefs[2] =
            parse_param(info, "steering_poly_a2", cfg_steer_.poly_coefs[2]);

        pid_controller_.initPid(
            cfg_pid_.kp, cfg_pid_.ki, cfg_pid_.kd, 1.0, -1.0, true
        );
        new_kp_ = cfg_pid_.kp;
        new_ki_ = cfg_pid_.ki;
        new_kd_ = cfg_pid_.kd;
    ...
    SpeedyHardwareInterface::on_activate(const rclcpp_lifecycle::State&)
    {
        RCLCPP_INFO(
            logger_,
            "[HARDWARE] Activating Direct RPi PWM Hardware Interface..."
        );
            ...
            ))
        {
            RCLCPP_FATAL(
                logger_,
                "[HARDWARE] Falha ao inicializar PWM do motor — abortando "
                "ativação."
            );
            motor_.stop();
            return hardware_interface::CallbackReturn::ERROR;
        ...
        param_node_->declare_parameter(
            "velocity_filter_alpha", cfg_pid_.filter_alpha
        );
        param_node_->declare_parameter(
            "pid_deadband_m_s", cfg_pid_.deadband_m_s
        );
        param_node_->declare_parameter(
            "motor_min_effort", cfg_motor_.min_effort
        );
        param_node_->declare_parameter(
            "max_linear_velocity", cfg_motor_.max_linear_velocity
        );
        param_node_->declare_parameter(
            "velocity_feedforward", cfg_motor_.velocity_feedforward
        );
        param_node_->declare_parameter(
            "motor_deadband_ms", static_cast<int>(cfg_motor_.deadband_ms)
        );
        param_node_->declare_parameter(
            "steering_center_pulse_ms", cfg_steer_.center_pulse_ms
        );
        param_node_->declare_parameter(
            "steering_max_deflection_ms", cfg_steer_.max_deflection_pulse_ms
        );
        param_node_->declare_parameter(
            "steering_max_speed_deg_per_sec", cfg_steer_.max_speed_deg_per_sec
        );
        param_node_->declare_parameter(
            "max_steering_angle_left_deg", cfg_steer_.max_angle_left_deg
        );
        param_node_->declare_parameter(
            "max_steering_angle_right_deg", cfg_steer_.max_angle_right_deg
        );
        param_node_->declare_parameter(
            "steering_trim_deg", cfg_steer_.trim_deg
        );
        param_node_->declare_parameter(
            "motor_invert_direction", cfg_motor_.invert_direction
        );
        param_node_->declare_parameter(
            "steering_invert_direction", cfg_steer_.invert_direction
        );
        param_node_->declare_parameter(
            "steering_use_polynomial", cfg_steer_.use_polynomial
        );
        param_node_->declare_parameter(
            "steering_poly_a0", cfg_steer_.poly_coefs[0]
        );
        param_node_->declare_parameter(
            "steering_poly_a1", cfg_steer_.poly_coefs[1]
        );
        param_node_->declare_parameter(
            "steering_poly_a2", cfg_steer_.poly_coefs[2]
        );
        param_node_->declare_parameter(
        ...
        front_odom_pub_ =
            param_node_->create_publisher<nav_msgs::msg::Odometry>(
                "/front_wheels/odom", rclcpp::QoS(10)
            );
        servo_pulse_pub_ =
            param_node_->create_publisher<std_msgs::msg::Float64>(
                "/steering/pulse_ms", rclcpp::QoS(10)
            );
        motor_cmd_pub_ = param_node_->create_publisher<std_msgs::msg::Float64>(
            "/motor/cmd_vel", rclcpp::QoS(10)
        );
        motor_raw_vel_pub_ =
            param_node_->create_publisher<std_msgs::msg::Float64>(
                "/motor/raw_vel", rclcpp::QoS(10)
            );
        motor_filtered_vel_pub_ =
            param_node_->create_publisher<std_msgs::msg::Float64>(
                "/motor/filtered_vel", rclcpp::QoS(10)
            );
        motor_effort_pub_ =
            param_node_->create_publisher<std_msgs::msg::Float64>(
                "/motor/effort", rclcpp::QoS(10)
            );

        param_cb_handle_ = param_node_->add_on_set_parameters_callback(
            std::bind(
                &SpeedyHardwareInterface::param_callback,
                this,
                std::placeholders::_1
            )
        );
        ...
        if (!servo_.init(cfg_pins_.servo))
        {
            RCLCPP_ERROR(
                logger_,
                "pigpio_start falhou — servo inativo (GPIO %d)",
                cfg_pins_.servo
            );
        }
        else
        {
            RCLCPP_INFO(
                logger_,
                "[HARDWARE] Servo GPIO %d centrado (1500us)",
                cfg_pins_.servo
            );
        }
        ...
        try
        {
            ...
            RCLCPP_INFO(
                logger_,
                "[HARDWARE] Enable da ponte H ativo em %s — EN(%d,%d)",
                chip_name.c_str(),
                cfg_pins_.motor_en_1,
                cfg_pins_.motor_en_2
            );
        ...
        {
            RCLCPP_ERROR(
                logger_, "GPIO enable (gpiod) init failed: %s", e.what()
            );
        }

        if (!encoder_.start(cfg_pins_.hall, cfg_pins_.hall_en))
            RCLCPP_ERROR(
                logger_, "Hall encoder init failed (GPIO %d)", cfg_pins_.hall
            );
        else
            RCLCPP_INFO(
                logger_,
                "[HARDWARE] Hall encoder ativo (GPIO %d)",
                cfg_pins_.hall
            );
        ...
        if (!encoder_fl_.start(cfg_pins_.hall_fl))
            RCLCPP_ERROR(
                logger_,
                "Hall encoder dianteiro esquerdo init failed (GPIO %d)",
                cfg_pins_.hall_fl
            );
        else
            RCLCPP_INFO(
                logger_,
                "[HARDWARE] Hall encoder dianteiro esquerdo ativo (GPIO %d)",
                cfg_pins_.hall_fl
            );
        ...
        if (!encoder_fr_.start(cfg_pins_.hall_fr))
            RCLCPP_ERROR(
                logger_,
                "Hall encoder dianteiro direito init failed (GPIO %d)",
                cfg_pins_.hall_fr
            );
        else
            RCLCPP_INFO(
                logger_,
                "[HARDWARE] Hall encoder dianteiro direito ativo (GPIO %d)",
                cfg_pins_.hall_fr
            );
    ...
    hardware_interface::CallbackReturn
    SpeedyHardwareInterface::on_deactivate(const rclcpp_lifecycle::State&)
    {
        motor_.stop();
        servo_.stop();
        encoder_.stop();
    ...
    hardware_interface::return_type SpeedyHardwareInterface::read(
    ...
    )
    {
        ...
        double gear_ratio, filter_alpha, wheel_radius, wheelbase,
            zero_timeout_s;
        int magnets_per_rev;
        {
        ...
        filtered_velocity_ = filter_alpha * raw_velocity_rad_s +
                             (1.0 - filter_alpha) * filtered_velocity_;
        ...
        filtered_vel_fl_ = filter_alpha * (d_fl * rad_per_pulse_front / dt) +
                           (1.0 - filter_alpha) * filtered_vel_fl_;
        filtered_vel_fr_ = filter_alpha * (d_fr * rad_per_pulse_front / dt) +
                           (1.0 - filter_alpha) * filtered_vel_fr_;

        t_idle_fl_ = (d_fl != 0) ? 0.0 : (t_idle_fl_ + dt);
        t_idle_fr_ = (d_fr != 0) ? 0.0 : (t_idle_fr_ + dt);
        if (motor_dir == 0 && t_idle_fl_ >= zero_timeout_s)
            filtered_vel_fl_ = 0.0;
        if (motor_dir == 0 && t_idle_fr_ >= zero_timeout_s)
            filtered_vel_fr_ = 0.0;
    ...
    hardware_interface::return_type SpeedyHardwareInterface::write(
        const rclcpp::Time& time,
        const rclcpp::Duration& period
    )
    {
        ...
        double omega_limit =
            (wheel_radius > 1e-6)
                ...
                : 1e9;
        double target_omega =
            std::clamp(current_velocity_cmd, -omega_limit, omega_limit);
        double effort = 0.0;

        double pid_velocity =
            (motor_.direction() < 0) ? -filtered_velocity_ : filtered_velocity_;
        double deadband_omega =
            (wheel_radius > 1e-6) ? (deadband_m_s / wheel_radius) : 0.0;
        if (std::abs(target_omega) >= deadband_omega)
        {
            ...
            double feedforward = (omega_limit > 1e-6)
                                     ? motor_cfg.velocity_feedforward *
                                           (target_omega / omega_limit)
                                     : 0.0;
            effort = feedforward +
                     pid_controller_.computeCommand(
                         error_linear,
                         period.nanoseconds() ? period.nanoseconds() : 20000000
                     );
        }
        else
        {
        ...
        motor_.apply(
            effort,
            time,
            motor_cfg.deadband_ms,
            motor_cfg.min_effort,
...
PLUGINLIB_EXPORT_CLASS(
    speedy_control::SpeedyHardwareInterface,
